		...
		switch ( $render ) {
			...
			case 'post_excerpt':
				...
				if ( \Bricks\Query::is_looping() ) {
					...
					if ( ! is_a( $loop_object, 'WP_Post' ) ) {
						...
						if ( $loop_object && ! empty( $loop_object->description ) ) {
							$value = \Bricks\Helpers::trim_words( $loop_object->description, $num_words, null, $keep_html );
						}
				...
				elseif ( is_archive() ) {
					...
					$element_name = ! empty( $element_data['element']['name'] ) ? $element_data['element']['name'] : '';
					...
					if ( $queried_object && ! empty( $queried_object->description ) && $element_name !== 'posts' ) {
						$value = \Bricks\Helpers::trim_words( $queried_object->description, $num_words, null, $keep_html );
				...
				$value = \Bricks\Helpers::get_the_excerpt( $post, $num_words, null, $keep_html );
				break;
			...
			case 'url_parameter':
				...
				if ( ! $value && ! empty( $url_components['query'] ) ) {
					...
					$value = isset( $parameters[ $parameter ] ) ? esc_attr( $parameters[ $parameter ] ) : '';
				}
