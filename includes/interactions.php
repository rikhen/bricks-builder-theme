...
class Interactions {
	...
	public function add_data_attributes( $attributes, $element ) {
		...
		if ( count( $interactions ) ) {
			...
			foreach ( $interactions as $element_interactions ) {
				...
				$animationType = ! empty( $element_interactions['animationType'] ) ? $element_interactions['animationType'] : false;
				...
				if ( stripos( $animationType, 'In' ) !== false ) {
					$hidden_on_load = true;
	...
	public function add_to_template_root( $attributes, $template_id ) {
		...
		if ( ! empty( $template_settings['template_interactions'] ) ) {
			...
			$json_interactions = json_encode( $interactions );
