...
class Builder {
	...
	public static function get_fonts() {
		...
		if ( is_array( $adobe_fonts ) && count( $adobe_fonts ) ) {
			...
			foreach ( $adobe_fonts as $adobe_font ) {
				if ( isset( $adobe_font['slug'] ) && isset( $adobe_font['name'] ) ) {
					$options[ $adobe_font['slug'] ] = $adobe_font['name'];
				}
			}
	...
	public static function get_google_fonts() {
		...
		if ( $google_fonts_generate ) {
			...
			foreach ( $google_fonts as $google_font ) {
				...
				$variants = ! empty( $google_font['variants'] ) ? json_encode( $google_font['variants'] ) : false;
			...
			$json = json_encode( $google_fonts_processed, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES );
	...
	public static function render_dynamic_data_on_elements( $elements, $post_id ) {
		if ( strpos( json_encode( $elements ), 'useDynamicData' ) === false ) {
			return $elements;
		}
