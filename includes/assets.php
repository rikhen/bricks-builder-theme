...
class Assets {
	...
	public static function enqueue_setting_specific_scripts( $settings = [] ) {
		...
		// STEP: Load Photoswipe for any lightbox setting (lightboxImage, lightboxVideo, Map 'infoImages', etc.)
		if ( strpos( $bricks_settings_string, '"lightbox' ) !== false || strpos( $bricks_settings_string, '"infoImages' ) !== false ) {
		...
		// STEP: Get inline CSS to load webfonts when using external files (set in element settings)
	...
	public static function generate_css_rules_from_setting( $settings, $setting_key, $setting_value, $controls, $selector, $css_type ) {
		...
		$has_dynamic_value = strpos( json_encode( $setting_value ), '"{' ) !== false;
	...
	public static function generate_css_from_element( $element, $css_type ) {
		...
		if ( in_array( $element['name'], [ 'section', 'container', 'block', 'div' ] ) && isset( $settings['hasLoop'] ) && ! Query::is_looping( $element['id'] ) ) {
